...
public class ScheduleGUI {
  ...
  public ScheduleGUI() {
    ...
    appFrame = new JFrame("Schedule Progress");
    ...
    JLabel title = new JLabel("Your Schedule Progress");
    ...
    addCategoryRow("學業", academicIcon, 60, Theme.ACADEMIC_COLOR, academicButton);
    addCategoryRow("興趣", hobbyIcon, 0, Theme.HOBBY_COLOR, hobbyButton);
    addCategoryRow("運動", sportIcon, 0, Theme.SPORT_COLOR, sportButton);
    addCategoryRow("社交", socialIcon, 0, Theme.SOCIAL_COLOR, socialButton);
    addCategoryRow("感情", relationshipIcon, 0, Theme.RELATIONSHIP_COLOR, relationshipButton);
  ...
  private CustomButton createDetailButton(ActionListener listener) {
    CustomButton btn = new CustomButton("細項觀看");
    btn.setFont(Theme.FONT_REGULAR);
    btn.setCornerRadius(15);
    btn.setColors(Theme.TEXT_MUTED, Theme.TEXT_MAIN); // Make them less prominent
    btn.addActionListener(listener);
    return btn;
  }

  private void addCategoryRow(
      String name, ImageIcon icon, int progress, Color color, CustomButton btn) {
  ...
  private class ButtonListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent a) {
      if (a.getSource() == academicButton) {
        new SubFacesSchedule(Faces.academic);
        appFrame.dispose();
